...
void key_schedule(const uint8_t key[16], uint32_t rk[32]) {
    uint32_t K[36];
    for (int i = 0; i < 4; i++)
    ...
    for (int i = 0; i < 32; i++) {
        ...
        tmp = tau(tmp);
        tmp ^= (tmp << 13 | tmp >> (32 - 13)) ^ (tmp << 23 | tmp >> (32 - 23));
        K[i + 4] = K[i] ^ tmp;
        rk[i] = K[i + 4];
    }
}
