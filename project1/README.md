...
- **计时功能**  
  ...
  由于本地硬件AMD Ryzen 7 5800H不支持GFNI以及VPROLD（AVX-512指令），故以下使用更低版本的AVX2实现：


## 1. 什么是 AVX2 并行化优化？
AVX2（Advanced Vector Extensions 2）是 Intel 推出的 SIMD 指令集扩展，支持 256 位向量运算

该优化将 SM4 算法中的核心运算转换为向量化操作，实现单条指令同时处理 8 个 32 位数据

通过重组算法流程，使多个独立数据块能够并行处理，大幅提高吞吐量

特别适合 ECB 等可并行加密模式，在支持 AVX2 的现代 CPU 上可获得显著加速

## 2. SM4 的 AVX2 并行化设计
### 2.1 并行架构设计
8 路并行：利用 256 位 YMM 寄存器同时处理 8 个 SM4 分组（128 位×8=1024 位）

混合执行模式：

对完整 8 分组块使用 AVX2 优化路径

对剩余分组回退到标量实现

统一接口：加密/解密使用相同计算核心，仅轮密钥顺序不同
...
__m256i newX0 = _mm256_xor_si256(X0, T_avx2(t));
```
S-Box 查表优化：

使用 _mm256_i32gather_epi32 指令实现并行查表

预先生成 32 位格式的 S-Box 表（SM4_SBOX32）

```c
__m256i g0 = _mm256_i32gather_epi32(SM4_SBOX32, i0, 4); // 并行查8个字节
```
线性变换 L：
...
```c
__m256i r2 = _mm256_slli_epi32(b, 2);
__m256i r10 = _mm256_slli_epi32(b, 10);
return _mm256_xor_si256(b, _mm256_xor_si256(r2, r10)); 
```
...
使用未对齐加载/存储指令（_mm256_loadu_si256）

Gather 指令优化不连续内存访问

#### 寄存器调度：

保持 4 个状态寄存器(X0-X3)在 YMM 寄存器中
